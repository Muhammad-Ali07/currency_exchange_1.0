...
@section('content')
    ...
    <section id="faq-search-filter">
        <form id="bank_currency_ledger_report" class="bank_currency_ledger_report faq-search-input" action="{{route('reports.bank_currency.currencyLedgerReport')}}" method="post" autocomplete="off">
            @method('post')
            @csrf
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-lg-12 text-center">
                            <h2 class="text-danger">{{ $data['title'] }}</h2>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-lg-4">
                            <span class="text-danger" style="font-size: 18px;font-weight: 900;">Filters</span>
                            <button class="btn btn-danger btn-sm" type="button" id="generate">Generate</button>
                        </div>
                    </div>
                    <!-- main title -->
                        ...
                        <div class="col-lg-12">
                            {{-- <form id="currency_ledger_report" class="currency_ledger_report faq-search-input" action="{{route('reports.currency.currencyLedgerReport')}}" method="post" enctype="multipart/form-data" autocomplete="off"> --}}
                                ...
                                <div class="row">
                                    <div class="col-lg-6">
                                        <div class="row">
                                            <label class="form-label" for="basic-default-password1"><b>Bank Currency Chart Code</b></label>
                                                <div class="col-sm-12">
...
<script>
    ...
    <script src="{{asset('/pages/help/bank_currency_chart_help.js')}}"></script>
    <script>
        $('#generate').click(function(){
            var chart_id = $('#chart_id').val();

            if (chart_id != '') {
                $("#bank_currency_ledger_report").submit();
            }else{
                $('#chart_name').addClass('was-validated000000000000');
            }
